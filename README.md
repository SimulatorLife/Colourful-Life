...
- **Profiling** — `node scripts/profile-energy.mjs` benchmarks the energy preparation loop. Adjust rows/cols via `PERF_ROWS`, `PERF_COLS`, `PERF_WARMUP`, `PERF_ITERATIONS`, and the stub `cellSize` with `PERF_CELL_SIZE` environment variables. Enable the heavier SimulationEngine benchmark with `PERF_INCLUDE_SIM=1` when you specifically need tick timings. For dense-tick profiling, use `PERF_INCLUDE_SIM=1 node scripts/profile-energy.mjs`; `simulationBenchmark.msPerTick` is a 10% trimmed mean and `rawMsPerTick` is the untrimmed mean. Use fresh processes and compare medians on the same machine.
  The current dense path uses scalar snapshots with deferred leaderboard entries, a single per-tick processing context, revision markers instead of per-tick `WeakSet` allocation, reusable mate/movement scratch buffers, and cached offscreen energy/density/age overlays keyed by the simulation tick. A paired Node 25/macOS run at 60x60, 90% seeded population, 30 measured ticks produced a **98.565 ms/tick** trimmed median (**107.481 ms/tick** raw median), versus **99.797 ms/tick** trimmed (**109.474 ms/tick** raw median) from a clean `038dc27` control: **1.2% lower trimmed time** and **1.8% lower raw time**. Both runs preserved `finalPopulation: 712` from `seededPopulation: 3240`. A separate 64x64 mock-canvas overlay run measured **0.1353 → 0.0034 ms/frame** (**39.3x**) for repeated unchanged cached frames; this is an in-process cache measurement, not a browser-frame guarantee. CPU profiling with `node --cpu-prof` continues to identify reproduction, target classification, DNA similarity, neural evaluation, and GC as the dominant dense-scenario costs. These figures are hardware- and scenario-specific; repeat the command when evaluating future changes.
  The latest structural scalability pass bounds neural mate previews to four nearest candidates after cheap genome scoring and skips grid/overlay painting on cadence-skipped animation frames. On the fixed 60x60, 65%-seeded, 20-warmup/30-measured-tick scenario, this reduced the trimmed mean from **86.60 ms/tick** to **63.18 ms/tick** (**27.0%**) and the raw mean from **92.26 ms/tick** to **67.06 ms/tick** (**27.3%**). See [`docs/performance-scaling.md`](docs/performance-scaling.md) for the method, tradeoffs, and interpretation.
- **Decision diagnostics** — Simulation decision traces are disabled by default so dense populations do not allocate full sensor, activation, and outcome records every tick. Set `decisionTelemetry: true` in simulation configuration when an inspector or experiment needs `Cell.getDecisionTelemetry()`; compact decision state still preserves neural reinforcement feedback.
