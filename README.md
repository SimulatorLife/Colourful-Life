...
8. Energy and an organism cannot share the same grid cell. When a resident occupies a tile, any stored tile energy must be absorbed into the organism, redistributed to neighbouring empty tiles, or dissipated so the grid never reports a resident and stored energy simultaneously.
