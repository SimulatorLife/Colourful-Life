...
- **Profiling** — `node scripts/profile-energy.mjs` benchmarks the energy preparation loop. Adjust rows/cols via `PERF_ROWS`, `PERF_COLS`, `PERF_WARMUP`, `PERF_ITERATIONS`, and the stub `cellSize` with `PERF_CELL_SIZE` environment variables. Enable the heavier SimulationEngine benchmark with `PERF_INCLUDE_SIM=1` when you specifically need tick timings.
  For dense-tick profiling, use `PERF_INCLUDE_SIM=1 node scripts/profile-energy.mjs`; the reported `simulationBenchmark.msPerTick` is a 10% trimmed mean and `rawMsPerTick` is the untrimmed mean. On the maintainer's Node 25/macOS run (60x60, 65% seeded population, 60 ticks), three fresh runs after the current structural changes measured **52.3 ms/tick** median trimmed mean (**57.9 ms/tick** raw median). A matched clean `HEAD` archive control measured **54.0 ms/tick** trimmed (**59.1 ms/tick** raw), so this slice measured about **3% lower trimmed mean** and **2% lower raw mean**; the larger historical **72.7 ms/tick** comparison reflects an earlier runtime state and should not be treated as a same-run guarantee. The dense scenario produced `finalPopulation: 383`; the sampling-cadence change intentionally stops one-cell churn from forcing expensive trait/diversity recomputation, so treat population output as a semantic regression invariant and review it alongside timing. These are hardware-specific comparison figures; repeat the command three times when evaluating future changes.
