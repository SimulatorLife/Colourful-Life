...
- **Profiling** — `node scripts/profile-energy.mjs` benchmarks the energy preparation loop. Adjust rows/cols via `PERF_ROWS`, `PERF_COLS`, `PERF_WARMUP`, `PERF_ITERATIONS`, and the stub `cellSize` with `PERF_CELL_SIZE` environment variables. Enable the heavier SimulationEngine benchmark with `PERF_INCLUDE_SIM=1` when you specifically need tick timings.
  For dense-tick profiling, use `PERF_INCLUDE_SIM=1 node scripts/profile-energy.mjs`; the reported `simulationBenchmark.msPerTick` is a 10% trimmed mean and `rawMsPerTick` is the untrimmed mean. On the maintainer's Node 25/macOS run (60x60, 65% seeded population, 60 ticks), three-run medians improved from **72.7 ms/tick** at `HEAD` to **59.4 ms/tick** after the hot-path changes (**18% lower trimmed mean**); final population remained 366 in every run. Treat these as hardware-specific comparison figures and repeat the command three times when evaluating future changes.
