...
- Neural mate selection blends brain forecasts with DNA courtship heuristics. Each cell scores its visible candidates with cheap genome-derived preferences, then previews reproduction sensors for a bounded set of nearest candidates before folding the brain's acceptance probability into their weights. This preserves neural influence while preventing dense populations from multiplying full network evaluation by the entire candidate pool; simpler genomes continue to lean on similarity heuristics.
...
- [`docs/performance-scaling.md`](performance-scaling.md) records the dense-population stress case, the bounded neural mate-selection design, and the render-loop budget separation.
- `pnpm run clean` delegates to Parcel's built-in clean routine to remove `dist/` and `.parcel-cache/` when the bundler cache becomes inconsistent.
