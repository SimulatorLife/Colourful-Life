...
export function drawOverlays(grid, ctx, cellSize, opts = {}) {
  ...
  if (showTraitOverlay && traitOverlay) {
    drawTraitFocusOverlay(grid, ctx, cellSize, traitOverlay);
  }
...
const TRAIT_OVERLAY_MIN_VISIBLE_INTENSITY = 0.025;
const TRAIT_OVERLAY_INTENSITY_BUCKETS = 36;
const TRAIT_OVERLAY_DEFAULT_STOPS = Object.freeze([
  Object.freeze({ value: 0, color: Object.freeze([12, 74, 110, 0]) }),
  Object.freeze({ value: 0.35, color: Object.freeze([56, 189, 248, 0.32]) }),
  Object.freeze({ value: 0.6, color: Object.freeze([129, 140, 248, 0.65]) }),
  Object.freeze({ value: 1, color: Object.freeze([236, 72, 153, 0.88]) }),
]);

function toRgbaString([r = 0, g = 0, b = 0, a = 1]) {
...
function normalizeColorStop(stop) {
  ...
  const value = clamp01(stop.value);
  const color = Array.isArray(stop.color) ? stop.color : null;

  if (!color || color.length < 3) return null;

  return { value, color: [color[0], color[1], color[2], color[3] ?? 1] };
}

function normalizeTraitColorStops(stops) {
  const rawStops =
    Array.isArray(stops) && stops.length > 0 ? stops : TRAIT_OVERLAY_DEFAULT_STOPS;
  const normalized = rawStops
    .map(normalizeColorStop)
    .filter(Boolean)
    .sort((a, b) => a.value - b.value);

  if (normalized.length === 0) {
    return TRAIT_OVERLAY_DEFAULT_STOPS.map((stop) => ({ ...stop }));
  }
  ...
  if (first.value > 0) {
    normalized.unshift({ value: 0, color: first.color.slice() });
  }

  if (last.value < 1) {
    normalized.push({ value: 1, color: last.color.slice() });
  }

  return normalized;
}

function sampleTraitColor(stops, value) {
  if (!Array.isArray(stops) || stops.length === 0) return null;

  const normalized = clamp01(value);

  if (normalized <= stops[0].value) {
    return toRgbaString(stops[0].color);
  }
  ...
  for (let i = 0; i < stops.length - 1; i++) {
    ...
    const t = clamp01((normalized - current.value) / span);
    const color = blendRgbaColor(current.color, next.color, t);

    return toRgbaString(color);
  }

  return toRgbaString(stops[stops.length - 1].color);
}

function quantizeTraitIntensity(value) {
  const normalized = clamp01(value);
  const step = TRAIT_OVERLAY_INTENSITY_BUCKETS;

  return Math.round(normalized * step) / step;
}

function formatTraitOverlayLabel(key) {
...
function drawTraitOverlayLegend(
  ctx,
  cellSize,
  cols,
  rows,
  { label, threshold, highlighted, population, mean, max, colorStops },
) {
  if (!ctx || !Number.isFinite(cols) || !Number.isFinite(rows) || !(cellSize > 0))
    return;

  const padding = 10;
  const gradientHeight = 12;
  const gradientWidth = clamp(cols * cellSize * 0.24, 140, 220);
  const textLineHeight = 14;
  const normalizedThreshold = clamp01(threshold);
  const safePopulation = Number.isFinite(population) ? population : 0;
  const safeHighlighted = Number.isFinite(highlighted) ? highlighted : 0;
  const highlightText =
    safePopulation > 0
      ? `${safeHighlighted}/${safePopulation} cells ≥ ${Math.round(normalizedThreshold * 100)}%`
      : `${Math.round(normalizedThreshold * 100)}% threshold`;
  const averageText = Number.isFinite(mean) ? `${mean.toFixed(2)}× avg` : null;
  const peakText = Number.isFinite(max) ? `${max.toFixed(2)}× peak` : null;
  const lines = [`${label ?? "Trait"} intensity`, highlightText];

  if (averageText || peakText) {
    const summary = [averageText, peakText].filter(Boolean).join(", ");

    if (summary) lines.push(summary);
  }

  lines.push("Tint strength tracks normalized gene output");

  const blockHeight = gradientHeight + padding * 3 + textLineHeight * lines.length;
  const blockWidth = gradientWidth + padding * 2;
  const x = padding;
  const y = padding;
  ...
  const stops = normalizeTraitColorStops(colorStops);

  stops.forEach((stop) => {
    gradient.addColorStop(stop.value, toRgbaString(stop.color));
  });
...
/**
 * Paints an overlay highlighting how strongly each living cell expresses the
 * selected trait. Intensities are derived from {@link Stats} trait definition
 * callbacks so UI and telemetry stay aligned.
 *
 * @param {import('../grid/gridManager.js').default} grid - Active grid manager.
 * @param {CanvasRenderingContext2D} ctx - Rendering context.
 * @param {number} cellSize - Size of a single grid cell in pixels.
 * @param {{
 *   key?: string,
 *   label?: string,
 *   compute: (cell: import('../cell.js').default) => number,
 *   threshold?: number,
 *   colorStops?: Array<{ value: number, color: number[] }>,
 * }} options - Trait overlay configuration.
 */
export function drawTraitFocusOverlay(grid, ctx, cellSize, options = {}) {
  if (!grid || !ctx || !(cellSize > 0)) return;

  const activeCells = grid.activeCells;

  if (!activeCells || activeCells.size === 0) return;

  const { compute } = options ?? {};

  if (typeof compute !== "function") return;

  const threshold = clamp01(options.threshold ?? 0.6);
  const label = typeof options.label === "string" ? options.label.trim() : null;
  const colorStops = normalizeTraitColorStops(options.colorStops);
  ...
  for (const cell of activeCells) {
    ...
    try {
      intensity = compute(cell);
    } catch (error) {
      if (typeof warnOnce === "function") {
        warnOnce("Trait overlay compute failed", error);
      }
      intensity = 0;
    }
    ...
    if (normalized >= threshold) highlighted++;

    if (normalized <= TRAIT_OVERLAY_MIN_VISIBLE_INTENSITY) {
      continue;
    }

    const bucket = quantizeTraitIntensity(normalized);
    let fillStyle = colorCache.get(bucket);

    if (!fillStyle) {
      fillStyle = sampleTraitColor(colorStops, bucket);
      if (fillStyle) colorCache.set(bucket, fillStyle);
    }
  ...
  if (population > 0) {
    drawTraitOverlayLegend(ctx, cellSize, grid.cols ?? 0, grid.rows ?? 0, {
      label: label ?? formatTraitOverlayLabel(options.key),
      threshold,
      highlighted,
      population,
      mean: sum / population,
