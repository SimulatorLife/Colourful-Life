...
test('defaultIsEventAffecting applies inclusive rectangle bounds', () => {
  ...
  assert.is(defaultIsEventAffecting(sampleEvent, 8, 4), false, 'rows beyond height excluded');
  assert.is(defaultIsEventAffecting(sampleEvent, 6, 7), false, 'cols beyond width excluded');
  assert.is(defaultIsEventAffecting(null, 6, 4), false, 'missing event ignored');
  assert.is(defaultIsEventAffecting({ type: 'flood' }, 6, 4), false, 'missing area ignored');
});
